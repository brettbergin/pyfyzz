...
class FuzzResultSerializer:
        ...
        for method_result in self.fuzz_results.method_results:
            ...
            for fuzz_case in method_result.test_cases:
                method_result_dict['test_cases'].append({
                    ...
                    'exception': str(fuzz_case.exception) if fuzz_case.exception else None,
    ...
    def as_dataframe(self):
        ...
        if 'exception' in df.columns:
            df['exception'] = df['exception'].apply(lambda x: str(x) if isinstance(x, Exception) else x)
        
        if 'inputs' in df.columns:
            df['inputs'] = df['inputs'].apply(lambda x: json.dumps(x) if isinstance(x, dict) else x)
        return df